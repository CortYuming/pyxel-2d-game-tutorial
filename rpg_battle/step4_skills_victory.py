import pyxel
import random

class Skill:
    def __init__(self, name, mp_cost, effect_type, power, description):
        self.name = name
        self.mp_cost = mp_cost
        self.effect_type = effect_type
        self.power = power
        self.description = description

class Character:
    def __init__(self, name, max_hp, max_mp, attack, defense):
        self.name = name
        self.max_hp = max_hp
        self.hp = max_hp
        self.max_mp = max_mp
        self.mp = max_mp
        self.attack = attack
        self.defense = defense
        self.is_defending = False
        self.skills = []
        
    def take_damage(self, damage):
        if self.is_defending:
            damage = max(1, damage // 2)
        self.hp = max(0, self.hp - damage)
        
    def heal(self, amount):
        self.hp = min(self.max_hp, self.hp + amount)
        
    def use_mp(self, amount):
        self.mp = max(0, self.mp - amount)
        
    def recover_mp(self, amount):
        self.mp = min(self.max_mp, self.mp + amount)
        
    def can_use_skill(self, skill):
        return self.mp >= skill.mp_cost
        
    def is_alive(self):
        return self.hp > 0
        
    def get_hp_ratio(self):
        return self.hp / self.max_hp
        
    def get_mp_ratio(self):
        return self.mp / self.max_mp

class Player(Character):
    def __init__(self):
        super().__init__("Hero", 100, 50, 25, 8)
        self.skills = [
            Skill("Fire Slash", 8, "damage", 35, "Fire-based attack"),
            Skill("Heal", 12, "heal", 30, "Recover HP"),
            Skill("Focus", 5, "buff", 0, "Recover MP")
        ]
        
    def draw(self, x, y):
        pyxel.rect(x, y, 32, 48, 1)
        pyxel.rect(x + 4, y + 4, 24, 40, 12)
        
        pyxel.pset(x + 10, y + 12, 0)
        pyxel.pset(x + 22, y + 12, 0)
        pyxel.line(x + 12, y + 20, x + 20, y + 20, 0)

class Enemy(Character):
    def __init__(self, name, max_hp, max_mp, attack, defense):
        super().__init__(name, max_hp, max_mp, attack, defense)
        self.skills = [
            Skill("Power Attack", 10, "damage", 25, "Powerful attack")
        ]
        
    def ai_action(self):
        if self.mp >= 10 and random.random() < 0.3:
            return "skill"
        elif random.random() < 0.6:
            return "attack"
        else:
            return "defend"
            
    def draw(self, x, y):
        pyxel.rect(x, y, 32, 48, 8)
        pyxel.rect(x + 4, y + 4, 24, 40, 9)
        
        pyxel.pset(x + 10, y + 12, 0)
        pyxel.pset(x + 22, y + 12, 0)
        pyxel.line(x + 12, y + 24, x + 20, y + 24, 0)

class BattleUI:
    def __init__(self):
        self.selected_action = 0
        self.selected_skill = 0
        self.actions = ["Attack", "Skill", "Defend"]
        self.menu_state = "main"
        
    def draw_character_status(self, character, x, y):
        pyxel.text(x, y, character.name, 7)
        pyxel.text(x, y + 10, f"HP: {character.hp}/{character.max_hp}", 7)
        pyxel.text(x, y + 20, f"MP: {character.mp}/{character.max_mp}", 7)
        
        bar_width = 60
        hp_ratio = character.get_hp_ratio()
        pyxel.rect(x, y + 30, bar_width, 6, 0)
        pyxel.rect(x + 1, y + 31, int((bar_width - 2) * hp_ratio), 4, 8)
        
        mp_ratio = character.get_mp_ratio()
        pyxel.rect(x, y + 38, bar_width, 6, 0)
        pyxel.rect(x + 1, y + 39, int((bar_width - 2) * mp_ratio), 4, 12)
        
    def draw_action_menu(self, player):
        if self.menu_state == "main":
            pyxel.rect(20, 140, 200, 50, 0)
            pyxel.rectb(20, 140, 200, 50, 7)
            
            for i, action in enumerate(self.actions):
                color = 10 if i == self.selected_action else 7
                pyxel.text(30 + i * 50, 155, action, color)
            
            pyxel.text(30, 170, "Arrow keys: Select  Space: Confirm", 7)
            
        elif self.menu_state == "skill":
            pyxel.rect(20, 120, 200, 70, 0)
            pyxel.rectb(20, 120, 200, 70, 7)
            
            for i, skill in enumerate(player.skills):
                color = 10 if i == self.selected_skill else 7
                if not player.can_use_skill(skill):
                    color = 5
                    
                pyxel.text(30, 130 + i * 12, f"{skill.name} (MP:{skill.mp_cost})", color)
                
            pyxel.text(30, 175, "Esc: Back  Space: Use", 7)
        
    def update_input(self, player):
        if self.menu_state == "main":
            if pyxel.btnp(pyxel.KEY_LEFT):
                self.selected_action = max(0, self.selected_action - 1)
            elif pyxel.btnp(pyxel.KEY_RIGHT):
                self.selected_action = min(len(self.actions) - 1, self.selected_action + 1)
            elif pyxel.btnp(pyxel.KEY_SPACE):
                if self.actions[self.selected_action] == "Skill":
                    self.menu_state = "skill"
                    return None
                else:
                    return self.actions[self.selected_action]
                    
        elif self.menu_state == "skill":
            if pyxel.btnp(pyxel.KEY_UP):
                self.selected_skill = max(0, self.selected_skill - 1)
            elif pyxel.btnp(pyxel.KEY_DOWN):
                self.selected_skill = min(len(player.skills) - 1, self.selected_skill + 1)
            elif pyxel.btnp(pyxel.KEY_SPACE):
                skill = player.skills[self.selected_skill]
                if player.can_use_skill(skill):
                    self.menu_state = "main"
                    return ("skill", skill)
            elif pyxel.btnp(pyxel.KEY_ESCAPE):
                self.menu_state = "main"
                
        return None

class Game:
    def __init__(self):
        pyxel.init(240, 200, title="RPG Battle - Step4")
        pyxel.mouse(True)
        self.player = Player()
        self.enemy = Enemy("Dragon", 120, 40, 28, 12)
        self.ui = BattleUI()
        self.turn = "player"
        self.battle_log = []
        self.game_over = False
        self.winner = None
        pyxel.run(self.update, self.draw)
        
    def update(self):
        if self.game_over:
            if pyxel.btnp(pyxel.KEY_SPACE):
                self.restart_battle()
            return
            
        if self.turn == "player":
            action = self.ui.update_input(self.player)
            if action:
                if isinstance(action, tuple):
                    self.execute_player_skill(action[1])
                else:
                    self.execute_player_action(action)
                    
        elif self.turn == "enemy":
            if pyxel.frame_count % 120 == 0:
                enemy_action = self.enemy.ai_action()
                self.execute_enemy_action(enemy_action)
                
    def execute_player_action(self, action):
        self.player.is_defending = False
        
        if action == "Attack":
            damage = max(1, self.player.attack - self.enemy.defense + random.randint(-3, 3))
            self.enemy.take_damage(damage)
            self.battle_log.append(f"Hero attacks! {damage} damage!")
            
        elif action == "Defend":
            self.player.is_defending = True
            self.player.recover_mp(3)
            self.battle_log.append("Hero defends and recovers MP...")
            
        self.check_battle_end()
        if not self.game_over:
            self.turn = "enemy"
            
    def execute_player_skill(self, skill):
        self.player.use_mp(skill.mp_cost)
        
        if skill.effect_type == "damage":
            damage = skill.power + random.randint(-5, 5)
            self.enemy.take_damage(damage)
            self.battle_log.append(f"Hero's {skill.name}! {damage} damage!")
            
        elif skill.effect_type == "heal":
            self.player.heal(skill.power)
            self.battle_log.append(f"Hero recovered {skill.power} HP!")
            
        elif skill.effect_type == "buff":
            self.player.recover_mp(15)
            self.battle_log.append("Hero focused and recovered MP!")
            
        self.check_battle_end()
        if not self.game_over:
            self.turn = "enemy"
            
    def execute_enemy_action(self, action):
        self.enemy.is_defending = False
        
        if action == "attack":
            damage = max(1, self.enemy.attack - self.player.defense + random.randint(-3, 3))
            self.player.take_damage(damage)
            self.battle_log.append(f"{self.enemy.name} attacks! {damage} damage!")
            
        elif action == "skill":
            if self.enemy.skills and self.enemy.can_use_skill(self.enemy.skills[0]):
                skill = self.enemy.skills[0]
                self.enemy.use_mp(skill.mp_cost)
                damage = skill.power + random.randint(-3, 3)
                self.player.take_damage(damage)
                self.battle_log.append(f"{self.enemy.name}'s {skill.name}! {damage} damage!")
            else:
                self.execute_enemy_action("attack")
                return
                
        elif action == "defend":
            self.enemy.is_defending = True
            self.enemy.recover_mp(5)
            self.battle_log.append(f"{self.enemy.name} is defending...")
            
        if len(self.battle_log) > 3:
            self.battle_log.pop(0)
            
        self.check_battle_end()
        if not self.game_over:
            self.turn = "player"
            
    def check_battle_end(self):
        if not self.player.is_alive():
            self.game_over = True
            self.winner = "Enemy"
            self.battle_log.append("Hero has fallen...")
        elif not self.enemy.is_alive():
            self.game_over = True
            self.winner = "Player"
            self.battle_log.append("Dragon defeated!")
            
    def restart_battle(self):
        self.player = Player()
        self.enemy = Enemy("Dragon", 120, 40, 28, 12)
        self.ui = BattleUI()
        self.turn = "player"
        self.battle_log = []
        self.game_over = False
        self.winner = None
        
    def draw(self):
        pyxel.cls(1)
        
        self.player.draw(40, 80)
        self.enemy.draw(160, 80)
        
        self.ui.draw_character_status(self.player, 20, 20)
        self.ui.draw_character_status(self.enemy, 140, 20)
        
        turn_text = "Player's Turn" if self.turn == "player" else "Enemy's Turn"
        pyxel.text(90, 5, turn_text, 7)
        
        for i, log in enumerate(self.battle_log):
            pyxel.text(10, 100 + i * 10, log, 7)
        
        if self.turn == "player" and not self.game_over:
            self.ui.draw_action_menu(self.player)
            
        if self.game_over:
            pyxel.rect(60, 80, 120, 40, 0)
            pyxel.rectb(60, 80, 120, 40, 7)
            pyxel.text(85, 90, f"{self.winner} Wins!", 11)
            pyxel.text(70, 105, "SPACE: Restart", 7)

Game()